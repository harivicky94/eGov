...
public class SearchPersonalRegisterAdaptor implements JsonSerializer<PersonalRegisterHelper> {
	...
	public JsonElement serialize(final PersonalRegisterHelper searchFormObj, final Type type,
		...
		if (searchFormObj != null) {
			...
			jsonObject.addProperty("dateOfAdmission", DateUtils.toDefaultDateFormat(searchFormObj.getDateOfAdmission()));
			jsonObject.addProperty("applicantName", defaultString(searchFormObj.getApplicantName()));
			jsonObject.addProperty("address1", defaultString(searchFormObj.getAddress1()));
			jsonObject.addProperty("address2", defaultString(searchFormObj.getAddress2()));
			jsonObject.addProperty("address3", defaultString(searchFormObj.getAddress3()));
			jsonObject.addProperty("surveyNo", defaultString(searchFormObj.getSurveyNo()));
			jsonObject.addProperty("village", defaultString(searchFormObj.getVillage()));
			...
			jsonObject.addProperty("totalFloorArea", searchFormObj.getTotalFloarArea());
			jsonObject.addProperty("noOfFloars", searchFormObj.getNoOfFloors());
			jsonObject.addProperty("far", searchFormObj.getFar());
			jsonObject.addProperty("currentStatus", searchFormObj.getCurrentStatus());
