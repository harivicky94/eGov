...
public class HolidayListController {
	...
	public String createHolidayList(@ModelAttribute(HOLIDAY) final Holiday holiday, final Model model,
		...
		Date d = new Date();
		d = holiday.getHolidaysTemp().get(0).getHolidayDate();
		if (!holidayListService.getPreLoadedGeneralHolidays(holidayListService.getCurrentYear())) {
		...
		for (Holiday hlday : holiday.getHolidaysTemp()) {
			hlday.setYear(holidayListService.getCurrentYear());
		}
